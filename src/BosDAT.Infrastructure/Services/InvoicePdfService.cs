...
public class InvoicePdfService : IInvoicePdfService
{
    ...
    private static void ComposeHeader(IContainer container, InvoiceDto invoice, SchoolBillingInfoDto schoolInfo)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(col => ComposeSchoolInfoColumn(col, schoolInfo));
            row.ConstantItem(200).AlignRight().Column(col => ComposeInvoiceSummaryColumn(col, invoice));
        });
    }

    private static void ComposeSchoolInfoColumn(ColumnDescriptor col, SchoolBillingInfoDto schoolInfo)
    {
    ...
    private static void ComposeInvoiceSummaryColumn(ColumnDescriptor col, InvoiceDto invoice)
    {
    ...
    private static void ComposeContent(IContainer container, InvoiceDto invoice, SchoolBillingInfoDto schoolInfo)
    {
        ...
        {
            col.Spacing(15);
            col.Item().Row(row => ComposeBillingRow(row, invoice));
            col.Item().Table(table => ComposeLineItemsTable(table, invoice));
            col.Item().AlignRight().Width(250).Column(totals => ComposeTotalsSection(totals, invoice, schoolInfo));
            col.Item().BorderTop(1).BorderColor(Colors.Grey.Lighten1).PaddingTop(10)
                .Column(footer => ComposePaymentSection(footer, invoice, schoolInfo));
        });
    }

    private static void ComposeBillingRow(RowDescriptor row, InvoiceDto invoice)
    {
    ...
    private static void ComposeLineItemsTable(TableDescriptor table, InvoiceDto invoice)
    {
    ...
    private static void ComposeTotalsSection(ColumnDescriptor totals, InvoiceDto invoice, SchoolBillingInfoDto schoolInfo)
    {
        totals.Spacing(4);
        ...
        var totalLabel = invoice.IsCreditInvoice ? "Totaal credit" : "Totaal te betalen";
        totals.Item().BorderTop(1).BorderColor(Colors.Black).PaddingTop(4).Row(r =>
        {
            r.RelativeItem().Text(totalLabel).Bold().FontSize(11);
    ...
    private static void ComposePaymentSection(ColumnDescriptor footer, InvoiceDto invoice, SchoolBillingInfoDto schoolInfo)
    {
