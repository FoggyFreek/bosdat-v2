    ...
    return (
      <div className="flex-1 flex items-center justify-center border rounded-lg bg-muted/30 p-8">
        <div className="text-center text-muted-foreground">
          ...
          <p className="text-sm">{t('enrollments.step2.noStudentsSelected')}</p>
          <p className="text-xs mt-1">{t('enrollments.step2.addStudentsHint')}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex-1 space-y-3 overflow-y-auto">
      <div className="flex items-center justify-between mb-2">
        <h3 className="font-medium text-sm">
          {t('enrollments.step2.selectedStudents', { count: students.length, max: maxStudents })}
        </h3>
      </div>
